    ...
    getData: function(callback, errorCallback){
                ...
                if(displayMap.TypeAsString != 'DateTime'){
                  ...
                  if(schemaMap.Type == 'Number'){
                    if(schemaMap.hasOwnProperty('Decimals')){
                      regex = new RegExp('^(\\d*).(\\d{'+schemaMap.Decimals+'})(\\d*)$');
                  ...
                  subFilter += displayMap.StaticName + " ge datetime'" +date.toISOString() + "' and " + displayMap.StaticName + " le datetime'" +nextDate.toISOString() + "'"
                }
              }
      ...
      + (this.state_map.filters.hasFilters
        ...
        + (this.config.isDocumentLibrary
          ? (filters.length > 0 ? ' and ' : '') + 'FSObjType eq ' + this.config.fileObjectType
          : '')
          : (this.config.isDocumentLibrary ? '&$filter=(FSObjType eq ' + this.config.fileObjectType + ')' : ''))
          + '&$top=5000';
